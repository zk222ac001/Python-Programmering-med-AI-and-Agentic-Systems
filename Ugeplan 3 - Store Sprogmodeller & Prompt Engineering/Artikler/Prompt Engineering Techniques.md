...
Prompt Engineering becomes much easier when students learn a small set of reusable techniques.

The following techniques are useful in Python programming, education, AI assistants, automation, and agentic systems.

```mermaid
flowchart TB
    A["🎯 Clear Task"] --> G["💡 Better Prompt"]
    B["🎭 Role Prompting"] --> G
    C["👥 Context / Audience"] --> G
    D["🧪 Zero-Shot"] --> G
    E["📚 Few-Shot Examples"] --> G
    F["📦 Output Format"] --> G
    H["🚧 Constraints"] --> G
    I["🧩 Task Decomposition"] --> G

    G --> J["🤖 More Useful AI Response"]
    ...
    style F fill:#cffafe,stroke:#0891b2,stroke-width:2px
    style H fill:#fee2e2,stroke:#dc2626,stroke-width:2px
    style I fill:#ffedd5,stroke:#ea580c,stroke-width:2px
    style G fill:#312e81,color:#ffffff,stroke:#1e1b4b,stroke-width:3px
    style J fill:#0f766e,color:#ffffff,stroke:#115e59,stroke-width:3px
```
...
A prompt should tell the AI exactly what task it must perform.
...
Use simple language and provide one executable example.
```

**Why it works:** The second prompt reduces ambiguity.

---

## 🟪 Technique 2 — Role Prompting

Role prompting gives the AI a professional perspective.
...
| Area | Example role |
|---|---|
| 🐍 Python | `You are an experienced Python teacher.` |
| 🛡️ Security | `You are a cybersecurity instructor.` |
| 🤖 AI | `You are an AI engineering tutor.` |
| 🌐 Networking | `You are a network engineer teaching beginners.` |
| 🧑‍💻 Software | `You are a senior software developer.` |

---

## 🟩 Technique 3 — Zero-Shot Prompting

**Zero-shot prompting** means asking the AI to perform a task without giving it an example first.

```text
Classify the following support ticket as:
...
Ticket:
"My application crashes when I press the login button."
```

The AI must understand the instruction directly.

### When to use it

- The task is simple.
- The instructions are clear.
- You do not need a strict example pattern.

---

## 🟨 Technique 4 — Few-Shot Prompting

**Few-shot prompting** gives the AI one or more examples before asking it to perform the task.
...
Example 2:
Message: "The app cannot connect to Wi-Fi."
Category: Technical
...
```mermaid
flowchart LR
    A["📚 Example 1"] --> D["🤖 AI Learns Pattern"]
    B["📚 Example 2"] --> D
    C["🆕 New Input"] --> D
    D --> E["✅ Consistent Output"]
...
Few-shot prompting is useful when the desired pattern is difficult to describe with instructions alone.

---

## 🟧 Technique 5 — Give Context
...
```text
Explain REST APIs to first-semester IT students.

They already understand Python functions but have never
worked with web APIs.

Use a simple weather application as the example.
```

The second prompt helps the AI choose the correct level and examples.
...
- Use beginner-friendly language.
- Include only one Python example.
...
## 🟦 Technique 7 — Specify the Output Format

You can tell the AI exactly how the answer should look.
...
Large tasks are easier when divided into smaller steps.

### Too broad

```text
Build an AI helpdesk application.
```

### Better

```text
Help me design an AI helpdesk application.

Work through these stages:

1. Define the requirements.
2. Design the ticket data model.
3. Create the Python project structure.
4. Build ticket classification.
5. Add an LLM response function.
6. Add error handling.
7. Create tests.
8. Write the README.

Handle one stage at a time.
```
...
## 🟩 Technique 9 — Ask for Evaluation and Refinement

Prompt Engineering is iterative.

After receiving an answer, ask the AI to evaluate or improve it.

```text
Review your previous explanation.

Check whether it is suitable for a first-semester IT student.

Improve any section that is too advanced.
```

Or:

```text
Evaluate this prompt using these criteria:
...
| Technique | Main purpose | Good for |
|---|---|---|
| 🎯 Clear instruction | Remove ambiguity | Almost every prompt |
| 🎭 Role prompting | Set perspective | Teaching, consulting, technical help |
| 🟩 Zero-shot | Perform task directly | Simple tasks |
| 📚 Few-shot | Show desired pattern | Classification, formatting, style |
| 👥 Context | Provide background | Personalized explanations |
| 🚧 Constraints | Set boundaries | Length, difficulty, scope |
| 📦 Output format | Control response structure | Tables, JSON, Markdown, reports |
| 🧩 Decomposition | Break down complex tasks | Projects, agents, coding |
| 🔁 Refinement | Improve results iteratively | Almost every complex task |
...
```mermaid
flowchart TD
    A["What do you need?"] --> B{"Simple task?"}
    B -->|Yes| C["🟩 Zero-Shot"]
    B -->|No| D{"Need a pattern?"}
    D -->|Yes| E["📚 Few-Shot"]
    D -->|No| F{"Large complex task?"}
    F -->|Yes| G["🧩 Decompose Task"]
    F -->|No| H["🎯 Clear Instructions + Context"]

    C --> I["📦 Add Output Format"]
    E --> I
    G --> I
    H --> I
    I --> J["🚧 Add Constraints if needed"]
    J --> K["🔁 Test and Refine"]

    style A fill:#312e81,color:#ffffff
    style C fill:#dcfce7,stroke:#16a34a
    style E fill:#fce7f3,stroke:#db2777
    style G fill:#ffedd5,stroke:#ea580c
    style H fill:#dbeafe,stroke:#2563eb
    style I fill:#cffafe,stroke:#0891b2
...
Do not generate the answer yet. Focus only on improving the prompt.
